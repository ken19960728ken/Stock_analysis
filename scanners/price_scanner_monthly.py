"""
月K線價格資料撈取 Scanner
從 Yahoo Finance 撈取全市場月K資料（近 3 年）
"""
import sys
...
class MonthlyPriceScanner(BaseScanner):
    name = "MonthlyPriceScanner"
    resume_tables = ["monthly_price"]
    ...
    def fetch_one(self, target):
        ...
        if index_exists("monthly_price", stock_id):
            return True

        df = yf.download(ticker, period="3y", interval="1mo", progress=False, auto_adjust=False)
        ...
        ok = save_to_db(save_df, "monthly_price", chunksize=1000)
        if ok:
            add_index("monthly_price", stock_id)
        self.limiter.wait()
        return ok
