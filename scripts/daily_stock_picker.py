...
# 選股器策略選用清單 + 權重
# ===================================================================
# 全系統共有 26 個策略（analysis/strategies/STRATEGY_MAP），但每日選股器
# 只採用下列 11 個。選用標準：**全市場回測掃描後，正報酬或參數優化後達標**
# 的策略才納入，權重依回測表現高低設定（RSI 反轉最高 1.0，其餘 0.5~0.8）。
#
# 為何只用 11 個（其餘 15 個未納入的原因）：
#   1. 純技術擇時類（MA 交叉、MACD、Bollinger、Parabolic SAR、Heikin-Ashi、
#      Dual Thrust）— 判斷單檔進出場時機，不適合做跨股票橫斷面排名選股，
#      且回測未達篩選門檻。
#   2. 部分基本面/籌碼（財報三率、自由現金流、融資融券訊號、股權集中度、
#      事件驅動）— 回測未達門檻，或訊號已被「多因子綜合」涵蓋。
#   3. 籌碼進階 4 策略（當沖情緒反轉、外資連續買超、散戶vs主力、官股護盤）
#      — 需要 day_trading / chip_broker / chip_gov_bank 三張表，但選股器的
#      批量快取 _load_all_tables 未載入這三表，故無法納入（屬資料層限制，
#      非回測淘汰）。
#
# 注意：基本面/籌碼策略（價值投資、法人跟單、多因子綜合、多策略動態組合、
# 營收動能、次產業輪動）的權重雖早已設定，但因 enrich dtype bug（2026-05
# 修復前 merge_asof 靜默失敗）長期恆 0 分，實際生效是從修復後才開始。
STRATEGY_WEIGHTS = {
    "RSI 反轉": 1.0,
    "價值投資": 0.8,
    "機器學習選股": 0.7,
